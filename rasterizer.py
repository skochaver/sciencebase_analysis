...
def do_the_thing(num_files):
    '''
    Does the thing.
    :param num_files: To keep track of the things as it does them.
    :return: This is just a runner function. It returns the satisfaction of a job well done.
    '''

    # This is just lazy directory stuff.
    shapefile_dir = os.path.join(os.getcwd(),'shape_outputs')

    # This is just lazy loop stuff.
    counter = 0
    for shp_file in os.listdir(shapefile_dir):
        name, ext = os.path.splitext(shp_file)
        if ext == ".shp":
            full_shp_path = os.path.join(shapefile_dir, shp_file)
            add_to_raster('empty_ras_4326.tif', full_shp_path)
            counter += 1
            print shp_file + '  ' + str(counter) + ' out of ' + str(num_files)
